...
class ProofOfInclusion:
    ...
    def test_poi_valid(self):
        ...
        for i in range(len(data)):
            ...
            self.assertTrue(
                self.trie.verify_proof_of_inclusion(proof),
...
class ProofOfExclusion:
    ...
    def test_poe_valid(self):
        ...
        keys = [
            [rlp.encode(str(i).encode()), str(i + 1).encode()] for i in range(101, 201)
        ]
    ...
    def test_poe_verify_one_item_removed(self):
        ...
        proofs = [
            self.trie.get_proof_of_exclusion(keccak_hash(rlp.encode(k))) for k in keys
        ]
        self.trie.delete(b"do")
        for proof in proofs:
...
class TestMptNonSecure(unittest.TestCase, ProofOfInclusion, ProofOfExclusion):
    ...
    def test_save_and_load_new_item_to_copy(self):
        ...
        self.assertNotEqual(
            self.trie.root_hash(),
            new_trie.root_hash(),
            "Root hashes are equal but should not be.",
        )
    ...
    def test_save_and_load_new_item(self):
        ...
        self.assertEqual(
            new_trie.root_hash(),
            new_trie.root_hash(),
            "Root hashes are not equal but should be.",
        )
